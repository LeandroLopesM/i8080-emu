...
{
    static std::variant<byte, word> reference_buffer = memory[0];
    ...
    if ((memory[pos++] & m_MOV) == m_MOV)
    {
        ...
        reference_buffer = resolve(memory[pos++] | m_MOV_DST << 2);
    } else if ((memory[pos++] & m_MVI) == m_MVI)
    {
        return {
            MVI,
            resolve(memory[pos++] | m_MVI_DST << 2),
            memory[0]
        };
    } else if ((memory[pos++] & m_LXI) == m_LXI)
    {
        return {
            LXI,
            resolve_rp(memory[pos++] | m_LXI_DST << 2),
            memory[0]
        };
